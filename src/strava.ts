...
async function getValidAccessToken(athleteId: number): Promise<string> {
  ...
  if (!record) throw new StravaAuthError(`No token found for athlete ${athleteId}`)

  const nowSeconds = Math.floor(Date.now() / 1000)
  if (record.expiresAt - nowSeconds >= 300) return record.accessToken
...
async function stravaRequest<T>(athleteId: number, url: string): Promise<T> {
  ...
  if (response.status === 429) {
    const retryAfter = parseInt(response.headers.get('X-RateLimit-Reset') ?? '900', 10)
  ...
  if (response.status === 404) throw new StravaNotFoundError()
...
export async function fetchStarredSegments(athleteId: number, perPage = 50): Promise<any[]> {
  return stravaRequest<any[]>(athleteId, `${STRAVA_BASE}/segments/starred?per_page=${perPage}`)
}

export async function fetchEffort(athleteId: number, effortId: string): Promise<any> {
  return stravaRequest<any>(athleteId, `${STRAVA_BASE}/segment_efforts/${effortId}`)
}
...
export async function fetchEffortStreams(
  ...
  for (const key of ['distance', 'altitude', 'velocity_smooth', 'heartrate', 'watts', 'cadence']) {
    if (raw[key]) {
      sliced[key] = {
        ...raw[key],
...
export async function fetchActivity(athleteId: number, activityId: string): Promise<any> {
  return stravaRequest<any>(athleteId, `${STRAVA_BASE}/activities/${activityId}`)
}
